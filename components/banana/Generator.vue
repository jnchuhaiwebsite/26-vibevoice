    ...
    <div class="container mx-auto px-4 relative z-10 pt-8">
      ...
      <div class="flex flex-col md:flex-row gap-8 lg:gap-12 items-stretch justify-center">
        ...
        <div class="w-full mx-auto md:mx-0 md:w-[45%] max-w-[720px] md:max-w-[560px] bg-banana-card-bg/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-banana-border-color/50 p-6 flex flex-col gap-4">
            ...
            <div class="w-full h-[150px] border-2 border-dashed border-banana-primary-yellow/60 rounded-xl p-4 hover:border-banana-primary-yellow transition-colors relative bg-banana-dark-bg">
              ...
              >
                ...
                <div class="text-center">
                                  <p class="text-sm text-banana-text-muted">Click or drag image here</p>
                <p class="text-xs text-banana-text-muted/70 mt-1">JPEG, PNG only. Max 10MB. Dims: >14px. Ratio: 1/3-3</p>
                </div>
              </div>
              <div v-else class="relative w-full h-full">
                ...
                <!-- 显示图片来源标识 -->
                <div v-if="!selectedImage" class="absolute -top-2 -left-2 bg-banana-secondary-blue text-white text-xs px-2 py-1 rounded-full">
                  Template
                </div>
              </div>
            </div>
          ...
          <!-- 质量等级 -->
          <div>
            <label for="quality" class="block text-lg font-semibold text-banana-text-light mb-2">
            ...
            <div class="flex items-center gap-4">
              ...
              />
              <span class="bg-gradient-to-b from-banana-dark-bg to-gray-900 text-banana-primary-yellow text-sm font-bold px-3 py-1 rounded-md w-16 text-center">{{ qualityLevel }}</span>
            </div>
          </div>
          
          <!-- 预设样式 -->
          <div class="preset-gallery pt-4">
            <p class="text-banana-text-muted text-center">Or, get inspired by a preset style:</p>
          ...
          <button @click="editImage" :disabled="loading || !mainPreview" class="relative w-full flex items-center justify-center gap-2 px-4 mt-6 lg:px-6 py-3 sm:py-4 bg-banana-primary-yellow/80 text-banana-dark-bg hover:bg-banana-primary-yellow rounded-full font-extrabold text-base sm:text-lg lg:text-xl shadow-xl transition transform hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <span class="absolute -top-2 -right-2 bg-banana-secondary-blue text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-white/50">1 credit</span>
          </button>
        </div>
        ...
        <div class="w-full  mx-auto md:mx-0 md:w-[55%] max-w-[720px] bg-banana-card-bg/50 backdrop-blur-sm rounded-3xl shadow-2xl border border-banana-border-color/50 p-4 relative overflow-hidden flex flex-col justify-center">
          ...
          <div v-if="loading" class="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-banana-dark-bg to-gray-900/50 rounded-lg">
              <svg class="animate-spin h-10 w-10 text-banana-primary-yellow" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
              <p class="text-white mt-4 text-lg">Creating your masterpiece with Nano Banana...</p>
          </div>
          ...
          <button 
            v-if="previewImage" 
            @click="downloadImage" 
            class="absolute top-6 right-6 bg-gradient-to-r from-banana-primary-yellow to-banana-secondary-blue text-banana-dark-bg font-bold py-2 px-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 flex items-center gap-2"
            :disabled="loading"
          >
            <svg 
              v-if="!loading" 
